...
- **Chế độ người dùng (user mode)** — nơi các chương trình thông thường chạy.
  ...
  không thể sửa bảng trang, không thể tắt interrupt. Nếu nó thử, phần cứng sẽ "trap"
  vào kernel.
...
kiểm soát này chính là **syscall** (lời gọi hệ thống), và nó là giao diện
...
- quản lý bộ nhớ vật lý và không gian địa chỉ ảo riêng cho từng tiến trình bằng
  phân trang Sv39;
- xử lý trap, interrupt và syscall;
- tạo tiến trình và chuyển đổi giữa chúng theo kiểu preemptive, trên nhiều CPU;
...
| `plic.go` | interrupt controller mức nền tảng (PLIC) | Ch. 7, 9 |
...
| `syscall.go` | bảng điều phối syscall | Ch. 7 |
...
- `a0`–`a7` — thanh ghi đối số / giá trị trả về. **Đối số của syscall đi
...
`mret` — một lệnh *return từ trap*. Phần cứng ngoan ngoãn "return" vào chế độ S
tại `main`, dù chẳng có trap nào từng xảy ra. Ta giả vờ một lần return để vào
được mức ta muốn.
...
- `mepc` / `sepc` — địa chỉ PC để tiếp tục khi return từ trap.
...
Khi xảy ra một syscall, một ngoại lệ, hay một device interrupt, phần cứng
...
return về mức trước đó. Mọi syscall, mọi nhịp đồng hồ, mọi disk interrupt đều
...
- **`unsafe` và các pragma (Phụ lục B).** Bảng trang, trapframe và thanh ghi thiết
  bị chỉ là những "view" có kiểu trên các địa chỉ vật lý. Ta sẽ dựa vào
...
func main() {
    if cpuid() == 0 {
        ...
        plicInitHart()    // Ch. 7/9 — xin PLIC các device interrupt
    ...
    scheduler()           // Ch. 10 — không bao giờ return
}
```
...
0x0C000000  PLIC   (device interrupt)
...
- **Ranh giới người dùng/kernel** được phần cứng thực thi bằng **các mức đặc
  quyền**, và **syscall** là cánh cửa hợp lệ duy nhất giữa chúng.
...
2. **Cú return giả.** Bằng lời của bạn, hãy giải thích vì sao việc vào chế độ
   supervisor lại được làm như một lần *return từ trap* (`mret`) thay vì một cú
